...
import { ServersCursorPage } from './v1';
...
export class Servers extends APIResource {
   ...
   * R2 URL.
   */
  create(params: ServerCreateParams, options?: RequestOptions): APIPromise<V1API.ServerResponse> {
    const { server } = params;
   ...
   */
  retrieve(id: string, options?: RequestOptions): APIPromise<V1API.ServerResponse> {
  ...
  ): APIPromise<V1API.ServerResponse> {
    const { server } = params ?? {};
  ...
  list(
    ...
    options?: RequestOptions,
  ): PagePromise<ServersCursorPage, V1API.Server> {
    return this._client.getAPIList('/api/v1/servers', CursorPage<V1API.Server>, { query, ...options });
  }
...
export interface ServerCreateParams {
  server: V1API.ServerCreate;
}

export interface ServerUpdateParams {
  server?: V1API.ServerUpdate;
}
